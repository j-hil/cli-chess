...
def def_coords_any(game: "GameState", attacker_coord: Vector | None) -> list[Vector]:
    ...
    if attacker.role is Role.PAWN:
        return _def_coords_pawn(game, attacker_coord)
...
def _def_coords_pawn(game: "GameState", attacker_coord: Vector) -> list[Vector]:
    """Helper function."""
    attacker = game[attacker_coord]
    result = []
